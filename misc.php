<section>
    <form>
        <input type="text" name="nome" id="nome">
        <label for="nome">Nome</label>
        <input type="submit" value="RECEBA!">

        <label for="senha">Senha</label>
        <input type="password" name="Senha" id="senha">
        <input type="submit" value="Enviar">

        <label for="num">numero</label>
        <input type="number" name="Numero" id="num">
        <input type="submit" value="Manda o zap">
        
       <fieldset>
        <legend>Qual sua linguagem favorita?</legend>
            <input type="radio" name="lang" id="html" value="html">
            <label for="html">HTML</label>


            <input type="radio" name="lang" id="css" value="css">
            <label for="css">CSS</label>
       </fieldset>
        
        <fieldset>
            <legend>Como você dar um rolê?</legend>
            <input type="checkbox" name="transporte[]" id="pe" value="pe">
            <label for="pe">A pé </label>

            <input type="checkbox" name="transporte[]" id="onibus" value="onibus">
            <label for="onibus">Onibus</label>

        </fieldset>
        <input type="submit" value="Conta ae">
        

        <label for="data">Data</label>
        <input type="date" name="data" id="data" max="2006-01-01">

        <label for="foto">Foto de perfil</label>
        <input type="file" name="foto" id="foto" accept="image/*">

        <input type="hidden" name="id" value="200">
        <input type="submit" value="Enviar">
    </form>
</section>
<div class="org-prod">
    <a href="produtos.html"> Blusas</a>
    <a href="produtos.html">Saias</a>
    <a href="produtos.html">Vestidos</a>
    <a href="produtos.html">Roupas intimas</a>
    <a href="produtos.html">Maquiagens</a>
    <a href="produtos.html">Bolsas</a>


</div>